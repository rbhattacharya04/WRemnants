    ...
    if args.makeMCefficiency:
        if dataset.group in common.background_MCprocs:
            if era == "2016PostVFP":
                ...
                df = df.Define("GoodTrigObjs", "wrem::goodMuonTriggerCandidate2018(TrigObj_id,TrigObj_pt,TrigObj_l1pt,TrigObj_l2pt,TrigObj_filterBits)")
        else:
            if era == "2016PostVFP":
                df = df.Define("GoodTrigObjs", "wrem::goodMuonTriggerCandidate(TrigObj_id,TrigObj_filterBits)")
            else:
                df = df.Define("GoodTrigObjs", "wrem::goodMuonTriggerCandidate2018(TrigObj_id,TrigObj_filterBits)")
        if era == "2016PostVFP":
            ...
            df = df.Define("passTrigger","HLT_IsoMu24 && wrem::hasTriggerMatch(goodMuons_eta0,goodMuons_phi0,TrigObj_eta[GoodTrigObjs],TrigObj_phi[GoodTrigObjs])")
    else:
        df = muon_selections.apply_triggermatching_muon(df, dataset, "goodMuons_eta0", "goodMuons_phi0", None, None, era)
